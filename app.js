...
function snapToRest(){if(!butterfly)return;butterfly.position.copy(restPoint);butterfly.rotation.set(0,0,-Math.PI/2);velocity.set(0,0,0);wings.forEach((wing,i)=>{wing.rotation.y=(i%2===0?-1:1)*1.47;});perchMesh.position.copy(restPoint).add(new THREE.Vector3(0,-.28,0));perchMesh.visible=true;}
function event(value,r,state='neutral',label='User interaction: novelty signal',penaltySignal=0){penalty=state==='aversive'?Math.min(.95,penalty*.55+penaltySignal):penalty*.62;reserve=state==='reward'?Math.min(.99,reserve*.9+r*.28):state==='aversive'?Math.max(0,reserve*.78-penaltySignal*.18):reserve*.94;if(state==='aversive'&&penalty>=.5){state='dormant';label+=': penalty >= 50%, descending into closed-wing rest';}reward=r;valence=state;const drive=Math.max(r,reserve*.82);arousal=state==='dormant'?8:state==='aversive'?Math.round(16+penalty*22):Math.round(16+drive*72);memory=Math.min(.99,memory*.72+r*.28+reserve*.08);manualHeat=Math.max(manualHeat,r,reserve);if(state==='dormant'){velocity.multiplyScalar(.08);perchMesh.visible=true;}pushLog(`${label} | ${value}`.slice(0,135),r,state,'USER');stepSlice();render();}
...
scene.fog=new THREE.FogExp2(0x030605,.032);renderer.setPixelRatio(1);renderer.setClearColor(0x030605,1);renderer.domElement.style.width='100%';renderer.domElement.style.height='100%';viewport.appendChild(renderer.domElement);camera.position.set(0,1.35,32);camera.lookAt(0,1.15,0);scene.add(butterfly);butterfly.add(body);butterfly.scale.setScalar(.54);
butterfly.position.set(-12,2.2,-4.5);
...
for(const y of[-.52,0,.52]){const leg=new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(.05,y,-.08),new THREE.Vector3(.55,y-.08,-.16)]),new THREE.LineBasicMaterial({color:0x9fb0aa,transparent:true,opacity:.86}));body.add(leg);}
...
const ground=new THREE.GridHelper(74,42,0x1e3b28,0x0c1610);ground.position.y=-3.1;ground.material.transparent=true;ground.material.opacity=.46;scene.add(ground);const world=new THREE.Group();scene.add(world);for(let i=0;i<58;i++){const height=.25+Math.random()*2.4,block=new THREE.Mesh(new THREE.BoxGeometry(.18+Math.random()*.45,height,.18+Math.random()*.45),dark);block.position.set((Math.random()-.5)*66,-3.1+height/2,(Math.random()-.5)*42);world.add(block);}const trail=new THREE.BufferGeometry().setFromPoints(Array.from({length:40},(_,i)=>new THREE.Vector3((i-20)*1.85,-2.8+Math.sin(i*.7)*.18,-8+Math.cos(i*.5)*4.8)));scene.add(new THREE.Line(trail,new THREE.LineBasicMaterial({color:0x274b36,transparent:true,opacity:.65})));
const bounds={x:18,yMin:.8,yMax:6.4,z:10.5},restPoint=new THREE.Vector3(-5.2,-2.72,1.4);
let target=new THREE.Vector3(12,3.5,4.2),velocity=new THREE.Vector3(.58,.05,.22),lastFrame=performance.now(),nextTurn=0,glideUntil=0,restTwitchAt=0,restTwitchUntil=0,wanderPhase=Math.random()*10;
function chooseTarget(now){const near=butterfly.position.distanceTo(target)<1.9;if(now<nextTurn&&!near)return;const rapid=valence==='reward';nextTurn=now+(rapid?2600:3800)+Math.random()*(rapid?2200:3600);if(Math.random()<.34)glideUntil=now+520+Math.random()*1100;const px=pointer.x/720*bounds.x*1.8-bounds.x*.9,py=-(pointer.y/570*4-2);if(rapid&&pointer.on&&Math.random()<.32)target.set(THREE.MathUtils.clamp(px,-bounds.x,bounds.x),THREE.MathUtils.clamp(py+2.8,bounds.yMin,bounds.yMax),-4+Math.random()*8);else if(valence==='aversive'&&pointer.on)target.set(THREE.MathUtils.clamp(-px*.75,-bounds.x,bounds.x),THREE.MathUtils.clamp(1.1+Math.random()*1.7,bounds.yMin,bounds.yMax),3.2+Math.random()*3.4);else target.set((Math.random()-.5)*bounds.x*2,bounds.yMin+Math.random()*(bounds.yMax-bounds.yMin),(Math.random()-.5)*bounds.z*2);}
function animate(now){const dt=Math.min(.05,(now-lastFrame)/1000);lastFrame=now;tick++;manualHeat*=.992;reserve=THREE.MathUtils.clamp(reserve-dt*.004,0,.99);const drive=Math.max(reward,reserve),arousalDrive=THREE.MathUtils.clamp(arousal/100,0,1);const resting=valence==='dormant'||valence==='warming';if(!resting){chooseTarget(now);const desired=target.clone().sub(butterfly.position);const wave=new THREE.Vector3(Math.sin(now*.0013+wanderPhase)*.18,Math.sin(now*.0021+1.7)*.2,Math.cos(now*.0011+wanderPhase)*.16);desired.normalize().add(wave).normalize();const linearSpeed=valence==='reward'?.85+arousalDrive*3.15:valence==='aversive'?.16+arousalDrive*.18:.42+arousalDrive*1.22;const speed=now<glideUntil?linearSpeed*.72:linearSpeed;velocity.lerp(desired.multiplyScalar(speed),Math.min(1,dt*(2.1+arousalDrive*1.4)));butterfly.position.addScaledVector(velocity,dt);butterfly.position.y+=Math.sin(now*.0037)*dt*.2;butterfly.position.x=THREE.MathUtils.clamp(butterfly.position.x,-bounds.x,bounds.x);butterfly.position.y=THREE.MathUtils.clamp(butterfly.position.y,bounds.yMin,bounds.yMax);butterfly.position.z=THREE.MathUtils.clamp(butterfly.position.z,-bounds.z,bounds.z);const bodyRoll=-Math.PI/2;const liftPitch=THREE.MathUtils.clamp(-velocity.y*.12,-.12,.12);const bank=THREE.MathUtils.clamp(-velocity.z*.08,-.1,.1);butterfly.rotation.x+=(liftPitch-butterfly.rotation.x)*dt*1.05;butterfly.rotation.y+=(bank-butterfly.rotation.y)*dt*1.05;butterfly.rotation.z+=(bodyRoll-butterfly.rotation.z)*dt*1.45;}else{butterfly.position.lerp(restPoint,dt*.82);butterfly.rotation.x+=(0-butterfly.rotation.x)*dt*1.2;butterfly.rotation.y+=(0-butterfly.rotation.y)*dt*1.2;butterfly.rotation.z+=(-Math.PI/2-butterfly.rotation.z)*dt*1.2;velocity.multiplyScalar(1-dt*5);perchMesh.position.copy(restPoint).add(new THREE.Vector3(0,-.28,0));perchMesh.visible=true;}if(!resting)perchMesh.visible=false;
  const frequency=valence==='reward'?5+arousalDrive*28:valence==='aversive'?.45+arousalDrive*.45:valence==='warming'?.03:valence==='dormant'?.012:1.8+arousalDrive*8.5;let flap=Math.sin(now*.001*frequency)*(valence==='reward'?.42+arousalDrive*.5:.2+arousalDrive*.3);if(valence==='reward')flap+=Math.sin(now*.001*(18+arousalDrive*22))*.12;if(valence==='warming')flap=0;if(valence==='dormant'){if(now>restTwitchAt){restTwitchAt=now+6500+Math.random()*11500;restTwitchUntil=now+260+Math.random()*460;}flap=1.47+(now<restTwitchUntil?Math.sin(now*.015)*.045:0);}wings.forEach((wing,i)=>{const direction=i%2===0?-1:1;const targetFlap=valence==='dormant'?direction*flap:direction*flap;wing.rotation.y+=(targetFlap-wing.rotation.y)*Math.min(1,dt*(valence==='reward'?9+arousalDrive*13:6));});
