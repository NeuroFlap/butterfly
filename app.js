...
function snapToRest(){if(!butterfly)return;butterfly.position.copy(restPoint);butterfly.rotation.set(0,Math.PI/2,0);velocity.set(0,0,0);wings.forEach((wing,i)=>{wing.rotation.y=(i%2===0?-1:1)*1.47;});perchMesh.position.copy(restPoint).add(new THREE.Vector3(0,-.28,0));perchMesh.visible=true;}
function event(value,r,state='neutral',label='User interaction: novelty signal',penaltySignal=0){penalty=state==='aversive'?Math.min(.95,penalty*.55+penaltySignal):penalty*.62;reserve=state==='reward'?Math.min(.99,reserve*.9+r*.28):state==='aversive'?Math.max(0,reserve*.78-penaltySignal*.18):reserve*.94;if(state==='aversive'&&penalty>=.5){state='dormant';label+=': penalty >= 50%, immediate closed-wing rest';}reward=r;valence=state;const drive=Math.max(r,reserve*.82);arousal=state==='dormant'?8:state==='aversive'?Math.round(16+penalty*22):Math.round(16+drive*72);memory=Math.min(.99,memory*.72+r*.28+reserve*.08);manualHeat=Math.max(manualHeat,r,reserve);if(state==='dormant'){velocity.set(0,0,0);wings.forEach((wing,i)=>{wing.rotation.y=(i%2===0?-1:1)*1.47;});perchMesh.visible=true;}pushLog(`${label} | ${value}`.slice(0,135),r,state,'USER');stepSlice();render();}
...
const bounds={x:18,yMin:.8,yMax:6.4,z:10.5},restPoint=new THREE.Vector3(-5.2,-1.9,1.4);
