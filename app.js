...
function animate(now){const dt=Math.min(.05,(now-lastFrame)/1000);lastFrame=now;tick++;manualHeat*=.992;reserve=THREE.MathUtils.clamp(reserve-dt*.004,0,.99);const drive=Math.max(reward,reserve),arousalDrive=THREE.MathUtils.clamp(arousal/100,.08,1);const resting=valence==='dormant'||valence==='warming';if(!resting){chooseTarget(now);const desired=target.clone().sub(butterfly.position);const wave=new THREE.Vector3(Math.sin(now*.0013+wanderPhase)*.28,Math.sin(now*.0021+1.7)*.24,Math.cos(now*.0011+wanderPhase)*.25);desired.normalize().add(wave).normalize();const baseSpeed=valence==='reward'?.78+arousalDrive*1.95:valence==='aversive'?.18:.38+arousalDrive*.46;const speed=now<glideUntil?baseSpeed*.42:baseSpeed;velocity.lerp(desired.multiplyScalar(speed),Math.min(1,dt*(valence==='reward'?1.9+arousalDrive*.7:1.05)));butterfly.position.addScaledVector(velocity,dt);butterfly.position.y+=Math.sin(now*.0037)*dt*.2;butterfly.position.x=THREE.MathUtils.clamp(butterfly.position.x,-bounds.x,bounds.x);butterfly.position.y=THREE.MathUtils.clamp(butterfly.position.y,bounds.yMin,bounds.yMax);butterfly.position.z=THREE.MathUtils.clamp(butterfly.position.z,-bounds.z,bounds.z);const yaw=Math.atan2(velocity.x,velocity.z);const bank=THREE.MathUtils.clamp(-velocity.x*.24,-.28,.28);const pitch=THREE.MathUtils.clamp(-velocity.y*.24,-.18,.18);butterfly.rotation.y+=(yaw-butterfly.rotation.y)*dt*.9;butterfly.rotation.z+=(bank-butterfly.rotation.z)*dt*1.15;butterfly.rotation.x+=(pitch-butterfly.rotation.x)*dt*1.1;}else{butterfly.position.lerp(restPoint,dt*1.25);butterfly.rotation.set(0,Math.PI/2,0);velocity.set(0,0,0);perchMesh.position.copy(restPoint).add(new THREE.Vector3(0,-.28,0));perchMesh.visible=true;}if(!resting)perchMesh.visible=false;
  const frequency=valence==='reward'?6+arousalDrive*20:valence==='aversive'?.5+arousalDrive*.7:valence==='warming'?.03:valence==='dormant'?.012:1.4+arousalDrive*4.2;let flap=Math.sin(now*.001*frequency)*(valence==='reward'?.46+arousalDrive*.44:.22+arousalDrive*.24);if(valence==='reward')flap+=Math.sin(now*.001*(18+arousalDrive*18))*.12;if(valence==='warming')flap=0;if(valence==='dormant'){if(now>restTwitchAt){restTwitchAt=now+6500+Math.random()*11500;restTwitchUntil=now+260+Math.random()*460;}flap=1.47+(now<restTwitchUntil?Math.sin(now*.015)*.045:0);}wings.forEach((wing,i)=>{const direction=i%2===0?-1:1;const targetFlap=valence==='dormant'?direction*flap:direction*flap;wing.rotation.y+=(targetFlap-wing.rotation.y)*Math.min(1,dt*(valence==='reward'?8+arousalDrive*10:6));});
